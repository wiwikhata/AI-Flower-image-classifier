...
trainloader = torch.utils.data.DataLoader(test_data,batch_size = 16, shuffle = True)
...
def build_model(data):
    ...
    return model
model = build_model(trainloader)
criterion = nn.NLLLoss()
optimizer = optim.Adam(model.classifier.parameters(), lr = 0.001)
# train_classifier   
def train_model(model, trainloader, validloader, criterion, optimizer, device='cuda'):
    ...
    for epoch in range(epochs):
        ...
        for ii, (inputs, labels) in enumerate(trainloader):
            ...
            if steps % print_every == 0:
                valid_loss, valid_accuracy = valid_cal_accuracy(model, validloader)
    ...
    return model 
# Calculate accuracy
def valid_cal_accuracy(model, validloader, device = 'cuda'):
...
train_model(model, trainloader, validloader, criterion, optimizer, device='cuda') 
...
# test the trained-model with the images prepared for testing. 
...
"output_size" : 102,
"arch"        : vgg16,
"hidden_layers": [each.out_feautures for each in model.hidden_layers],   
"optimizer" : optimizer,
