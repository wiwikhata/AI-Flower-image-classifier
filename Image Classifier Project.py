...
#only training for classifier parameters
optimizer = optim.Adam(model.classifier.parameters(), lr = 0.001)
model.to(device)
epochs, steps, running_loss, print_every = 8, 0, 0, 40
    ...
    for ii,(images,labels) in enumerate(trainloader):
        ...
        if steps % print_every == 0:
           ...
           "Epoch:{}/{}...".format(e + 1, epochs),
            "Training_Loss:{:.3f}".format(test_loss/print_every),
...
def process_image(image):
    ...
    if height > width:
        i_mage.thumbnail((256,height/width * 256))
...
img = Image.open(valid/flowers)

 Imageshow(process_image(img))
